...
pub fn gc(s: &mut State){
    ...
    for v in 0..s.stack.len(){
        let v1 = s.stack[v].clone();
        //check for Address
        match v1 {
            Val::Vaddr(b) => {
                base = b.clone();
                let v2 = s.heap[base as usize];
                //check for sizes
                match v2 {
                    Val::Vsize(n) => {
                        ...
                        for i in base..(base + size as u32 + 1){
                            new_heap.push(s.heap[i as usize]).clone();
                        }
                        //update next
                        next += size as u32;
                    },
            ...
            _ => (),    //do nothing just keep looping
        }
    }
    ...
    while scan < next{
        let v1 = new_heap[scan as usize].clone();
        //check for addrs
        match v1 {
            Val::Vaddr(b) => {
                base = b.clone();
                addresses.insert(base, base);
                //check size
                let v2 = new_heap[base as usize];
                match v2 {
                    Val::Vsize(n) => {
                        size = n.clone();
                        for i in base..(base + size as u32 + 1){
    ...
    match i{
        ...
        Instr::Alloc =>{
            ...
            if addr + size as usize >= 1024{
                ...
                //pass state
                gc(s);
                //check if size is still too big
            }
